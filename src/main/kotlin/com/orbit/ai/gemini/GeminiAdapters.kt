            ...
            mapOf(
                ...
                "generationConfig" to mapOf(
                    ...
                    "responseSchema" to mapOf(
                        ...
                        "required" to listOf(
                            // **전부 필수다.** 선택으로 두면 이 모델이 응답을 제대로
                            // 만들지 않는다. 실측 결과 name·mainCategory 만 필수일 때는
                            // 본문이 통째로 비거나(파싱 실패 → "읽을 수 없습니다") 세부
                            // 분류에 깨진 문자열이 들어왔고, 전부 필수로 바꾸자 여덟 항목이
                            // 모두 사리에 맞게 채워졌다.
                            //
                            // "강제하면 근거 없이 채운다"는 우려로 한때 선택으로 두었는데,
                            // 실제로는 그 반대였다. 근거가 약할 때의 대비는 스키마가 아니라
                            // 프롬프트가 한다 — 위에 "사진에서 보이는 근거로 가장 그럴듯한
                            // 값을 고르라"고 적어 두었고, 사용자는 저장 전에 고칠 수 있다.
 ...
 * 전신 사진에 옷을 합성한다.
 *
 * **프롬프트를 일부러 짧게 유지한다.** 얼굴을 지키려고 지시를 쌓았더니 정확히
 * 반대로 작용했다. 배경 교체·비율 지정·구도 지시를 하나씩 더할 때마다 모델은
 * "편집"이 아니라 "새로 생성"하는 쪽으로 가고, 그 과정에서 얼굴이 재구성된다.
 * 3154자짜리 프롬프트가 765자짜리보다 나빴다.
 *
 * 출력 크기가 그 증거였다. 최소 변경 프롬프트는 입력(655x1600)과 사실상 같은
 * 656x1602 를 냈고(= 편집), 긴 프롬프트는 1152x922 를 냈다(= 재생성). 얼굴도
 * 전자만 원본과 구별되지 않았다.
 *
 * **출력 크기가 이 기능의 회귀 신호다.** 결과가 입력 비율을 따라가지 않으면
 * 얼굴은 이미 다른 사람이다. 사람 눈으로 얼굴을 비교하기 전에 크기부터 봐라.
 *
 * 그래서 배경을 그대로 둔다. 배경을 지우면 얼굴을 잃는다 — 둘 다는 안 된다.
 * 이 앱의 값어치는 "내 모습에 옷만 바뀐 것"이므로 얼굴이 우선이다.
 *
 * 다만 프롬프트만으로는 **모자란다.** 짧게 줄인 뒤에도 편집과 재생성 사이를
 * 오갔다. 남은 흔들림은 아래 generationConfig 의 temperature 0 이 잡는다.
 * 짧은 프롬프트가 방향을 정하고, temperature 0 이 그 방향을 매번 고르게 한다.
 *
 *
    ...
    override fun generate(bodyPhoto: ByteArray, items: List<ByteArray>): ByteArray {
        val parts = buildList {
            add(
                ...
                textPart(
                    /*
                     * **이 문구는 검증된 그대로다. 늘리지 마라.**
                     *
                     * 한 번 실제로 이렇게 틀렸다. 짧은 프롬프트가 통하는 것을 확인한 뒤
                     * 코드로 옮기면서 "레이어 순서", "색·소재를 그대로 재현", 강조 표시를
                     * 덧붙여 263자를 517자로 만들었다. 전부 맞는 말이고 전부 해로웠다.
                     * 앱에서 3회 연속 1152x922(재생성), 같은 사진·같은 설정으로 이 문구를
                     * 직접 보내면 3회 연속 656x1602(편집)였다. 차이는 문장 길이뿐이었다.
                     *
                     * 지시를 하나 더할 때마다 모델은 "고쳐라"에서 "만들어라"로 옮겨가고,
                     * 만드는 순간 얼굴은 원본의 얼굴이 아니게 된다. 여기서 보태고 싶은
                     * 문장이 생기면, 보탠 뒤 출력 크기가 입력과 같은지부터 확인해라.
                     * 세로가 가로로 바뀌었다면 그 문장이 얼굴을 지운 것이다.
                     */
                    """
                    一枚目の写真の女性は、そのままの人物だ。
        ...
        val response = client.generateContent(
            ...
            mapOf(
                ...
                "generationConfig" to mapOf(
                    ...
                    /*
                     * temperature 0. 이게 없으면 **같은 입력에도 결과가 갈린다.**
                     *
                     * 프롬프트를 위와 같이 줄인 뒤에도, 직접 호출에서는 656x1602(입력과
                     * 같은 프레임 = 편집)가 나오는데 앱에서 부르면 1152x922(가로로 새로
                     * 그림 = 재생성)가 나오는 일이 있었다. 입력이 같으니 원인은 샘플링의
                     * 무작위성이고, 재생성 쪽으로 굴러떨어지는 순간 얼굴을 잃는다.
                     *
                     * 0 으로 내리면 모델이 가장 확률 높은 해석을 고르는데, 그 해석은
                     * "주어진 사진을 고쳐라"다. 실제로 이 값을 넣은 뒤 출력이 입력 크기를
                     * 따라가고(655x1600 → 656x1602) 배경·자세·표정이 그대로 남았다.
                     *
                     * 비율을 imageConfig.aspectRatio 로 박는 방법도 시험했는데, 그건
                     * 오히려 768x1376 로 다시 그린다. 지정한 비율에 맞추려면 화면을
                     * 다시 짜야 하므로 편집이 아니라 생성이 되어 버린다. 그래서 비율은
                     * 지정하지 않고, 무작위성만 없앤다.
                     */
                    "temperature" to 0,
                ),
